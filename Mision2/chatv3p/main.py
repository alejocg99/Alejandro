from flask import Flask, render_template, request, jsonify
from chatbot.data import training_data
from chatbot.model import build_and_train_model, load_model, predict_answers
app = Flask(__name__)
model, vectorizer, unique_answers = load_model()
if model is None:
    model,vectorizer,unique_answers = build_and_train_model(training_data)
@app.route("/")
def home():
    return render_template("index.html")
@app.route("/chat", methods=["POST"])
def chat():
    user_tex = request.form.get("message","")
    if not user_tex.strip():
        return jsonify({"response":"Por favor responde 🤖"})
    response = predict_answers(model,vectorizer,unique_answers,user_tex)
    return jsonify({"response": response})
if __name__== "__main__":
    app.run(host="0.0.0.0", port=5000)


